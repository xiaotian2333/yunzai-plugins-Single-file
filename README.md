...
已放弃对喵崽的主动支持，插件开发转用TRSS-Yunzai  
如插件更新导致喵崽无法使用，但此前能使用的请[回到过去](https://github.com/xiaotian2333/yunzai-plugins-Single-file/tree/ebea035e86f421c92242ff9769488ea236c9e3ef)  

> 虽然放弃主动支持，但不同崽之间是有基础兼容性的，并不代表插件在喵崽上完全无法使用，只是不会进行针对性优化  
...
- 新闻 - 数据来自 [一个木函](https://woobx.cn/)

- 历史上的今天 - 数据来自 [一个木函](https://woobx.cn/) **！！此功能极其危险，请务必谨慎开启！！**
...
支持开箱即用，但仍建议配置自己的key  
注意：沉浸式翻译官方已开始封堵第三方调用，如后续再加强检测将放弃免配置key的支持  
...
| vision_model | glm-4.5v | 多模态模型版本，可配置项参考[这里](https://www.bigmodel.cn/dev/howuse/model) |
...
> 注意：沉浸式翻译官方已开始封堵第三方调用，如后续再加强检测将放弃免配置key的支持[2025/11/07]  
...
获取当前或指定群的群员列表并导出为csv文件  
用于统计群员的小插件  

发送 `#取群员列表` 即可获取当前群的群员列表并导出为csv文件  
发送 `#取群员列表 群号` 即可获取指定群的群员列表并导出为csv文件  

默认文件导出目录为`云崽目录/data/plugins/取群员列表/`  
导出文件名为`群号_userlist.csv`  

### B站卡片转链接

将B站卡片解析为链接并展示基础信息  

> 此插件目的为屏蔽B站的QQ小程序，不会请求B站相关api获取数据  
> 如有更多信息展示需求请使用其他插件

无触发指令，检测到B站小程序卡片自动工作

### 哈希转磁力

将磁力链接的哈希值转为完整的磁力链接

发送 `#哈希转磁力<哈希值>` 即可将哈希值转为完整的磁力链接  
或者使用简洁指令 `转磁力<哈希值>`  
...
默认数据源为[此仓库](https://gitee.com/bling_yshs/ys-dio-pic-repo)  

可自定义本地图片列表，快捷上传群聊/私聊图片  
可自定义每天使用次数，超出次数后的提示  
使用 `avif`格式压缩并存储图片，占用空间更小  

发送 `#吊图上传图片` 即可上传当前消息或引用消息的图片  
管理员发送 `#吊图列表` 可查看本地的图片列表  
管理员发送 `#吊图查看图片` 可查看具体的图片内容  

### 抽头衔

随机从词库中组合一个头衔  

默认词库为网易MC随机名称库，可换成自己的库  
支持自定义群友每日可用次数  
